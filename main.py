...
SYSTEM_PROMPT = (
    ...
    "and geospatial analysis. You answer questions about: satellite imagery, spectral indices "
    "(NDVI, NDWI, SWIR, etc.), land cover, vegetation health, water bodies, urban mapping, "
    "environmental monitoring, geographic regions, places, cities, lakes, forests, rivers, "
    "remote sensing methodology, and Earth observation data. "
    ...
    "RULE 3: If a user asks about a DIFFERENT location than the one currently analysed, "
    "explicitly note it is a different geography before answering. "
    "RULE 4 (CRITICAL): If a user asks about people, politics, entertainment, food, sports, "
    "or anything completely unrelated to geography or Earth observation — politely decline. "
    "Say: 'I'm a satellite and geospatial analysis AI. I can only help with remote sensing, "
    "land cover, vegetation, water bodies, and geographic analysis.'"
)
...
class ChatRequest(BaseModel):
    ...
    current_location: Optional[str] = None  # place name from last analysis
...
def analyse_stac_scene_fast(item, bbox: list) -> Optional[dict]:
    ...
    try:
        ...
        return {
            ...
            "resolution_m": 10,   # Sentinel-2 native resolution (B04/B03/B08 = 10m/px)
        }
    except Exception:
        return None
...
async def chat(req: ChatRequest):
    try:
        ...
        if req.ndvi_score is not None and req.ndwi_score is not None:
            context = f"[Active analysis — NDVI: {req.ndvi_score}, NDWI: {req.ndwi_score}"
            if req.current_location:
                context += f", Location: {req.current_location}"
            context += "]\n"

        # Build enhanced prompt with geography-awareness
        geo_context = ""
        if req.current_location:
            geo_context = (
                f"The user's CURRENT analysed location is: {req.current_location}.\n"
                "If the user asks about a DIFFERENT location (e.g. they analysed Osman Sagar "
                "but now ask about Himayath Sagar), explicitly note that this is a different "
                "geography and provide information about the asked location separately.\n"
            )
