...
let lastNdvi = null;
let lastNdwi = null;
let lastLocation = null;  // tracks the last analysed place name for chat context
...
async function runAutoFetch() {
  ...
  try {
    ...
    const resText = lastScene.resolution_m ? ` · ${lastScene.resolution_m}m/px` : '';
    document.getElementById('sensor-display').textContent = `SENSOR: Sentinel-2 L2A · AUTO-FETCH${resText}`;
    lastLocation = data.display_name || place;
...
// ── Chat ───────────────────────────────────────────────────────────────────
// Hard geo-domain keyword check — only blocks clearly non-geo queries
// (politics, people, entertainment, food, sports etc.)
const NON_GEO_PATTERNS = [
  /\b(modi|gandhi|trump|biden|president|prime minister|politician|actor|actress|movie|film|cricket|football|recipe|cook|food|restaurant|song|music|dance|fashion|celebrity|bollywood|hollywood)\b/i,
  /\b(how are you|what are you|who are you|tell me a joke|write a poem|write code|debug|programming)\b/i,
];

function isGeoRelated(msg) {
  // Block if matches obvious non-geo patterns
  return !NON_GEO_PATTERNS.some(p => p.test(msg));
}
...
async function sendChat() {
  ...
  // Hard client-side check — block obvious off-topic queries
  if (!isGeoRelated(msg)) {
    appendBubble(
  ...
  try {
    ...
    if (lastLocation) body.current_location = lastLocation;
